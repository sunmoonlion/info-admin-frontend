...
IMAGE_REGISTRY="${ADMIN_CSR_IMAGE_REGISTRY:-harbor.sunmoonai.com:30443}"
IMAGE_PROJECT="${ADMIN_CSR_IMAGE_PROJECT:-k8s-images}"
...
# ── Harbor 凭证（从环境变量读取，未设置则提示输入）──────────────────────────
_harbor_user="${HARBOR_USER:-}"
_harbor_pass="${HARBOR_PASSWORD:-}"
if [ -z "$_harbor_user" ]; then
    read -rp "Harbor 用户名: " _harbor_user
fi
if [ -z "$_harbor_pass" ]; then
    read -rsp "Harbor 密码: " _harbor_pass
    echo
fi
