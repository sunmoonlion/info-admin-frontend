...
| `src/layouts/default.vue`      | `app/components/app-shell.tsx` + `interface-settings.tsx` | 桌面侧栏、移动 Drawer、顶部、面包屑、标签、界面设置和内容出口      |
...
| Pinia tabs/settings store      | `app/store/ui.ts`                                         | 仅标签、侧栏、主题、密度和语言等 UI 偏好可持久化；固定标签不可误关 |
...
| route meta / menu tree         | `app/lib/navigation.ts`                                   | 菜单、面包屑、标签和体验级角色过滤共享一份元数据                   |
