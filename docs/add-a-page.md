...
在 `app/lib/navigation.ts` 增加路径、稳定 key、message key、图标、固定标签和可选 `requiredRoles`。菜单、面包屑、标签和前端体验级权限过滤必须消费同一份元数据；不要在页面或 Shell 中复制菜单数组。

`requiredRoles` 只改善浏览器导航体验，不是安全边界。后端仍必须对每个 API 和资源执行最终授权；A2.2 会把真实 `/auth/me` 身份与该映射接通。
