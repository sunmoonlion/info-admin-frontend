  ...
  it('accepts the RFC 3339 UTC offset emitted by the FastAPI backend', async () => {
    ...
    await expect(
      loadBrowserSession({
        ...
        cookieHeader: 'sunmoonai_info_admin_sid=opaque',
  ...
  it('forwards only the cookie and correlation context and accepts the exact safe DTO', async () => {
    ...
    await expect(
      loadBrowserSession({
        ...
        cookieHeader: 'sunmoonai_info_admin_sid=opaque-session',
        correlationId: 'correlation-1234',
        expectedApp: 'info',
        fetchImpl,
      }),
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      new URL('http://info-backend:8000/api/auth/admin/me'),
      expect.objectContaining({
        cache: 'no-store',
        redirect: 'manual',
        headers: {
          Accept: 'application/json',
          Cookie: 'sunmoonai_info_admin_sid=opaque-session',
